...
object syntax extends LowerPrioritySyntax {
  ...
  type EnumerableOf[@specialized E] = Enumerable with Elements[E]
  ...
  implicit def addOps[K](lhs: AddKeys[K] with ElementsK[K]): AddOps[K, lhs.type] = new AddOps[K, lhs.type](lhs)

  implicit def searchableAddOps[K](lhs: AddKeys[K] with Searchable[K] with ElementsK[K]): SearchableAddOps[K] = new SearchableAddOps(lhs)
