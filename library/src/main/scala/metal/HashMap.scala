...
abstract class HashMap[K, V] extends metal.Map[K, V] {
  ...
  def mutableCopy = new metal.mutable.HashMap[K, V](keys.clone, buckets.clone, values.clone, size, used, mask, limit)
