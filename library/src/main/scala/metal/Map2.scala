...
object Map2 {
  ...
  @inline @tailrec final def isSubset[K, V1, V2](lm: Map2[K, V1, V2], rm: Map2[K, V1, V2])(lp: Ptr[lm.Tag]): Boolean = lp.asInstanceOf[Ptr[lm.Tag]] match {
    case VPtr(lvp) =>
      val k = lm.ptrKey(lvp)
      rm.ptrFind(k) match {
  ...
  @inline @tailrec final def hash[K, V1, V2](m: Map2[K, V1, V2])(p: Ptr[m.Tag], h: Int): Int = p.asInstanceOf[Ptr[m.Tag]] match {
    case VPtr(vp) =>
      val k = m.ptrKey(vp)
...
trait Map2[K, V1, V2] extends Searchable[K] with Countable with Values1[V1] with Values2[V2] { lhs =>
...
trait MMap2[K, V1, V2] extends Map2[K, V1, V2] with AddKeys[K] with Removable[K] with Updatable1[V1] with Updatable2[V2] { lhs =>
