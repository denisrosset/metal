...
trait FMap2[K, V1, V2] extends FColl with Searchable[K] with Enumerable with Values1[V1] with Values2[V2] with NElements3[K, V1, V2] with JavaMethods[FMap2[K, V1, V2]] { lhs =>
  ...
  def keyArray(ptr: VPtr[FMap2.this.type]): Array[K]
  def keyIndex(ptr: VPtr[FMap2.this.type]): Int
  def value1Array(ptr: VPtr[FMap2.this.type]): Array[V1]
  def value1Index(ptr: VPtr[FMap2.this.type]): Int
  def value2Array(ptr: VPtr[FMap2.this.type]): Array[V2]
  def value2Index(ptr: VPtr[FMap2.this.type]): Int

  def ptrHash(ptr: VPtr[FMap2.this.type]): Int = {
  ...
  def ptrToString(ptr: VPtr[FMap2.this.type]): String = {
  ...
  final def ptrEquals(thisPtr: VPtr[FMap2.this.type], that: FMap2[K, V1, V2]): Boolean =
