...
abstract class SetCheck[A: ClassTag, LB, Extra[_]](factory: MSetFactory[LB, Extra])(implicit extra: Extra[A], lbev: A <:< LB)
  ...
  def hybridEq(d: MetalSet[A], s: mutable.Set[A]): Boolean =
    d.size == s.size && {
      @tailrec def rec(p: Ptr[d.Tag]): Boolean = (p.asInstanceOf[Ptr[d.Tag]]) match {
        case VPtr(vp) =>
...
class BooleanSetCheck extends AutoSetCheck[Boolean, Any, Dummy](HashSet)
class IntHashSetCheck extends AutoSetCheck[Int, Any, Dummy](HashSet)
class IntBitSetCheck extends SetCheck[Int, Int, Dummy](BitSet) {
  def A: Arbitrary[Int] = Arbitrary(Gen.choose(0, 10000))
}
class IntSortedSetCheck extends AutoSetCheck[Int, Any, Order](SortedSet)
class StringHashSetCheck extends AutoSetCheck[String, Any, Dummy](HashSet)
class StringSortedSetCheck extends AutoSetCheck[String, Any, Order](SortedSet)
